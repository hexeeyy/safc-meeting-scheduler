...
export default function ReusableCalendar({
  ...
  const generateRecurringEvents = useCallback((event: CalendarEvent): CalendarEvent[] => {
    if (event.recurrence !== 'monthly') return [event];

    const recurringEvents: CalendarEvent[] = [];
    const startDate = new Date(event.start);
    const endDate = new Date(event.end);
    const dayOfMonth = startDate.getDate();
    const maxMonths = 12; // Generate events for the next 12 months

    for (let i = 0; i < maxMonths; i++) {
      const newStart = addMonths(startDate, i);
      newStart.setDate(dayOfMonth);
      newStart.setHours(startDate.getHours(), startDate.getMinutes(), 0, 0);

      const newEnd = addMonths(endDate, i);
      newEnd.setDate(dayOfMonth);
      newEnd.setHours(endDate.getHours(), endDate.getMinutes(), 0, 0);

      const now = new Date();
      if (newStart < now) continue; // Skip past dates
  ...
  const allEvents = events.flatMap(generateRecurringEvents);
  ...
  const meetings = allEvents.filter((e) => !e.canceled);
