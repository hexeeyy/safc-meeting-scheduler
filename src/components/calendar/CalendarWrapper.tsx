...
export default function CalendarWrapper({
  ...
  const moveEvent = useCallback(
    ({ event, start, end }: { event: CalendarEvent; start: Date | string; end: Date | string }) => {
      ...
      if (event.recurrence === 'monthly') {
        alert('Cannot move recurring events individually. Edit the event to change the time for all recurrences.');
        return;
      }
  ...
  const handleSelectSlot = useCallback(
    ...
    [setModalOpen, setPendingSlot, hasTimeConflict]
  );
